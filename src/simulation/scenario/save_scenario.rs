use crate::simulation::asset::serialization::{
    SerializedBody, SerializedBodyData, SerializedFixedBodyFrame, SerializedLightSource,
    SerializedMat3, SerializedVec, SimulationData,
};
use crate::simulation::components::body::{
    BodyChildren, BodyRotation, BodyShape, LightSource, Mass, ModelPath, RotationSpeed,
    SimPosition, Star, Velocity,
};
...
pub fn save_scenario(mut system_panel_set: SystemPanelSet) {
    ...
    system_panel_set
        .toasts
        .0
        .add(success_toast("Scenario saved"));
}
...
fn collect_bodies(system_panel_set: &SystemPanelSet) -> Vec<SerializedBody> {
    ...
    system_panel_set
        .bodies
        .iter()
        .filter(|(_, _, _, _, _, _, _, _, _, _, _, _, star)| star.is_some())
        .for_each(|(entity, _, _, _, _, _, _, _, _, children, _, _, _)| {
            let mut data = find_body_data(system_panel_set, entity)
                .map(|(data, _)| data)
                .unwrap();
...
fn collect_planets(
    ...
    for planet_entity in children.0.clone() {
        if let Some((planet_data, planet_children)) =
            find_body_data(system_panel_set, planet_entity)
        {
            let moons = collect_moons(system_panel_set, planet_children.unwrap().clone());
            planets.push(SerializedBody {
                children: moons,
                data: planet_data.clone(),
            });
        }
    }
    planets
...
fn collect_moons(system_panel_set: &SystemPanelSet, children: BodyChildren) -> Vec<SerializedBody> {
    ...
    for child_entity in children.0.clone() {
        if let Some(child_data) =
            find_body_data(system_panel_set, child_entity).map(|(data, _)| data)
        {
            moons.push(SerializedBody {
                children: Vec::new(),
...
fn find_body_data(
    system_panel_set: &SystemPanelSet,
    entity: Entity,
) -> Option<(SerializedBodyData, Option<BodyChildren>)> {
    system_panel_set
        .bodies
        .iter()
        .find(|(e, _, _, _, _, _, _, _, _, _, _, _, _)| *e == entity)
        .map(
            |(_, m, p, v, n, mp, d, rs, _at, child, naif, rotation, _)| {
                (
                    create_serialized_body_data(
                        m.0,
                        p.current / 1000.0,
                        v.0 / 1000.0,
                        n.to_string(),
...
fn find_light_source(
    ...
    system_panel_set
        .lights
        .iter()
        .find(|s| s.parent == entity)
        .map(|s| SerializedLightSource {
