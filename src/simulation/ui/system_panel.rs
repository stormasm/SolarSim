...
use crate::simulation::components::editor::{
    CreateBodyState, CreateBodyType, EditorSystemType, EditorSystems,
};
...
use bevy::prelude::{
    AabbGizmoConfigGroup, Camera, Commands, Entity, GizmoConfigStore, KeyCode, Name, NextState,
    Query, Res, ResMut, Visibility, With, Without,
};
...
pub struct SystemPanelSet<'w, 's> {
    ...
    star_query: Query<
        ...
        (With<Star>, Without<Planet>, Without<Planet>),
    >,
    planet_query: Query<
        'w,
        's,
    ...
    moon_query: Query<
        'w,
        's,
        (&'static Name, Entity, &'static mut Visibility),
        (With<Moon>, Without<Planet>, Without<Star>),
    >,
            ...
            .show(egui_context.ctx_mut(), |ui| {
                ScrollArea::vertical().auto_shrink(true).show(ui, |ui| {
                    ...
                    for (s_name, s_children, s_entity, _) in &mut system_panel_set.star_query {
                        let s_old_selected =
                            system_panel_set.selected_entity.entity == Some(s_entity);
                        let mut s_selected = s_old_selected;
                        let new_create_body = body_tree(
                            s_entity,
                            ui,
                            &mut s_selected,
                            s_name,
                            ...
                            |ui| {
                                for planet_child in &s_children.0 {
                                    if let Ok((p_name, p_children, p_entity, _)) =
                                        system_panel_set.planet_query.get_mut(*planet_child)
                                    {
                                        let p_old_selected =
                                            system_panel_set.selected_entity.entity
                                                == Some(p_entity);
                                                ...
                                                for moon_child in &p_children.0 {
                                                            ...
                                                            .get_mut(*moon_child)
                                                    {
                                                        ...
                                                        if m_selected && !m_old_selected {
                                                            system_panel_set
                                                                .selected_entity
                                                                .change_entity(m_entity, ctrl_hold);
                                                        }
                                                    }
                                                }
                                            },
                                        ...
                                        if p_selected && !p_old_selected {
                                            system_panel_set
                                                .selected_entity
                                                .change_entity(p_entity, ctrl_hold)
                                        }
                                        if new_create_body.is_some() {
                                            *system_panel_set.create_body_state =
                                                new_create_body.unwrap();
                                            system_panel_set.commands.run_system(
                                                system_panel_set.systems.0
                                                    [EditorSystemType::CREATE_BODY],
                                            );
                                        }
                                    }
                                }
                        ...
                        if s_selected && !s_old_selected {
                            system_panel_set
                                .selected_entity
                                .change_entity(s_entity, ctrl_hold)
                        }
                        if new_create_body.is_some() {
                            *system_panel_set.create_body_state = new_create_body.unwrap();
                            system_panel_set.commands.run_system(
                                system_panel_set.systems.0[EditorSystemType::CREATE_BODY],
                            );
                        }
                    }
                    if *system_panel_set.sim_state_type == SimStateType::Editor
                        && ui
                            .button("+")
                            .on_hover_text("Create new major body")
                            .clicked()
                    {
                        *system_panel_set.create_body_state = CreateBodyState {
                        ...
                        system_panel_set
                            .commands
                            .run_system(system_panel_set.systems.0[EditorSystemType::CREATE_BODY]);
                    }
                    ui.separator();
                    ui.heading("Options");
                    ...
                    if skybox_enabled && !skybox_setting {
                        ...
                        system_panel_set.commands.entity(entity).insert(Skybox {
                            image: system_panel_set.cubemap.image_handle.clone(),
                            brightness: 1000.0,
                            ..default()
                        });
                    ...
                    ui.checkbox(
                        &mut system_panel_set.billboard.dynamic_hide,
                        "Dynamically hide body names",
                    );
                    if *system_panel_set.sim_state_type == SimStateType::Simulation {
                        ui.checkbox(
                            &mut ui_state.dyn_hide_orbit_lines,
                            "Dynamically hide orbit lines",
                        );
                    }
                    ...
                    ui.add_space(5.0);
                    if *system_panel_set.sim_state_type == SimStateType::Editor
                        && ui.button("Edit metadata").clicked()
                    {
