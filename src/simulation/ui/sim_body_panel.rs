...
use crate::simulation::components::body::{
    BodyChildren, BodyParent, BodyShape, Mass, OrbitSettings, RotationSpeed, SimPosition, Velocity,
};
...
use crate::simulation::ui::components::{
    body_multiplier_field, body_property_field, copy_value_button,
};
...
pub struct SimBodyPanelSet<'w, 's> {
    ...
    query: Query<
        ...
        (
            ...
            &'static BodyShape,
            &'static mut OrbitSettings,
            &'static mut Mass,
            &'static mut Transform,
            Option<&'static mut ApsisBody>,
            Option<&'static BodyChildren>,
            Option<&'static BodyParent>,
            &'static AniseMetadata,
        ),
    >,
...
pub fn sim_body_panel(mut set: SimBodyPanelSet) {
    ...
    if let Some(entity) = set.selected_entity.entity {
        ...
        {
            if children.is_some() && children.unwrap().0.contains(&entity) {
                //check for the parent of the selected entity
                parent = Some((pos, velocity, name, mass.clone(), meta));
            } else {
                if b_entity == entity {
                    //check for the selected entity
                    selected = Some((
                        name,
                        b_entity,
                        pos,
                ...
                } else if let Some(parent_id) = maybe_parent {
                    //check for potential children of the entity
                    ...
                    ScrollArea::vertical().auto_shrink(true).show(ui, |ui| {
                        ...
                        if let Some((_, _, _, p_mass, _)) = parent {
                            if let Some(apsis) = apsis {
                                let distance = ((apsis.aphelion.distance
                                    + apsis.perihelion.distance)
                                    / 2.0) as f64;
                                orbit.period = 2.0
                                    * std::f64::consts::PI
                                    * f64::sqrt(
                                        f64::powf(distance, 3.0) / (G * (p_mass.0 + mass.0)),
                                    );
                        ...
                        let c_distance_in_units =
                            camera_pos.translation.distance(transform.translation) as f64;
                        ui.label(format!(
                            "{}",
                            format_length(set.s_scale.unit_to_m_32(c_distance_in_units as f32))
                        ));
                        ui.label(format!(
                            "{:.3} au",
                            set.s_scale.unit_to_m(c_distance_in_units) * M_TO_AU as f64
                        ));
                        ...
                        if s_children.iter().count() > 0 {
                            let old_draw_children_orbits =
                                s_children.iter().all(|(_, orbit)| orbit.draw_lines);
                        ...
                        ui.label(RichText::new("Arrows").size(16.0).underline());
                        ...
                        if !orbit.auto_scale_arrows {
                            ui.label(RichText::new("Scale").size(14.0));
                            ui.add(
...
fn shape_section(ui: &mut Ui, shape: &BodyShape) {
    ...
    ui.vertical(|ui| {
        ui.label(format!(
            "Polar Radius (km): {}",
            shape.ellipsoid.polar_radius_km
        ));
        ui.label(format!(
            "Semi major equatorial radius (km): {}",
            shape.ellipsoid.semi_major_equatorial_radius_km
        ));
        ui.label(format!(
            "Semi minor equatorial radius (km): {}",
            shape.ellipsoid.semi_minor_equatorial_radius_km
        ));
    });
}
...
fn orbit_section(
    ui: &mut Ui,
    pos: &SimPosition,
    parent_pos: &SimPosition,
    orbit: &mut OrbitSettings,
    p_name: &str,
    new_apsis: Option<Mut<ApsisBody>>,
) {
    ui.label(
        RichText::new(format!("Distance to {} (Center)", p_name))
            .size(16.0)
            .underline(),
    );
    ...
    if let Some(mut apsis) = new_apsis {
        //Apsis
        ui.label(
            RichText::new(format!("Periapsis ({})", p_name))
                .size(16.0)
                .underline(),
        );
        ...
        ui.label(
            RichText::new(format!("Apoapsis ({})", p_name))
                .size(16.0)
                .underline(),
        );
...
fn mass_section(
    ...
    toast_container: &mut ToastContainer,
) {
...
fn velocity_section(
    ...
    let velocity_prefix = if parent.is_some() { "Orbital" } else { "Total" };
    ui.label(
        RichText::new(format!("{} Velocity", velocity_prefix))
            .size(16.0)
            .underline(),
    );
    ui.horizontal(|ui| {
