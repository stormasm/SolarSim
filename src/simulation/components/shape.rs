...
use bevy::{
    ...
    math::Vec3A,
    prelude::{
        in_state, Children, GlobalTransform, IntoScheduleConfigs, Query, Res, ResMut, Transform,
        Update, Vec3, With,
    },
...
impl Plugin for DiameterPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<SavedAabbs>().add_systems(
            Update,
            apply_real_diameter.run_if(in_state(SimState::Loading)),
        );
    }
}
...
pub fn apply_real_diameter(
    ...
    if !bodies.is_empty() && bodies.iter().all(|(_, _, _, diameter, _)| diameter.applied) {
    ...
    for (children, name, handle, mut shape, mut transform) in &mut bodies {
            ...
            if !shape.applied {
                match asset_server.get_load_state(&handle.0) {
                    ...
                    Some(a) => match a {
                        ...
                        LoadState::Loaded => {}
                        LoadState::Failed(e) => {
                            toasts.0.add(important_error_toast(
                                format!("Failed to load asset for body '{}': {}", name.as_str(), e)
                                    .as_str(),
                            ));
                            shape.applied = true;
                        }
        ...
        for children in children {
            if let Ok(scene) = scenes.get(*children) {
                ...
                let semi_major_radius_units = s_scale.m_to_unit_32(
                    (shape.ellipsoid.semi_major_equatorial_radius_km * 1000.0) as f32,
                ); // km to meters
                let semi_minor_radius_units = s_scale.m_to_unit_32(
                    (shape.ellipsoid.semi_minor_equatorial_radius_km * 1000.0) as f32,
                );
                let polar_radius_units =
                    s_scale.m_to_unit_32((shape.ellipsoid.polar_radius_km * 1000.0) as f32);
...
fn calculate_aabb(
    meshes: QueryManyIter<
        (&GlobalTransform, Option<&Aabb>),
        With<Mesh3d>,
        impl Iterator<Item = Entity> + Sized,
    >,
) -> Aabb {
