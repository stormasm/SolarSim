use crate::simulation::components::body::{
    BodyChildren, BodyShape, Moon, OrbitSettings, Planet, Velocity,
};
...
use bevy::{
    app::{App, Plugin},
    prelude::{Entity, Gizmos, IntoScheduleConfigs, Query, Transform, With},
};
...
impl Plugin for DirectionPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            PostUpdate,
            (display_force_and_velocity.after(PanOrbitCameraSystemSet))
...
fn display_force_and_velocity(
    ...
    for (transform, _, orbit, shape, velocity) in &planet_query {
        ...
        if orbit.display_velocity {
            gizmos.arrow(
                ...
                transform.translation
                    + (velocity.0.normalize() * multiplier * orbit.arrow_scale as f64).as_vec3(),
                css::RED,
            );
        }
    }
    for (entity, transform, orbit, shape, velocity) in &moon_query {
        ...
        if orbit.display_velocity {
            if let Some((_, _, _, _, vel)) = planet_query
                .iter()
                .find(|(_, ch, _, _, _)| ch.0.contains(&entity))
            {
                velocity_arrow(
                    transform,
                    (velocity.0 - vel.0).normalize(),
                    multiplier,
                    &mut gizmos,
                    &css::RED.into(),
                );
...
fn multiplier(
    ...
    if orbit.auto_scale_arrows {
        f64::max(
            cam.translation.distance(transform.translation) as f64 / 6.0,
            d,
        )
    } else {
...
fn force_arrow(
    ...
    gizmos.arrow(
        transform.translation,
        transform.translation + (direction * scale).as_vec3(),
        *color,
    );
}
