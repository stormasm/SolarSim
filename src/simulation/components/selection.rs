...
use bevy::prelude::{
    in_state, Entity, IntoScheduleConfigs, Query, ResMut, Resource, Transform, Vec3, With,
};
...
impl Plugin for SelectionPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<SelectedEntity>().add_systems(
            Update,
            (apply_camera_to_selection.after(SimulationStep)/*.before(pan_orbit_camera)*/)
    ...
    if let Some(entity) = selected_entity.entity {
        if let Err(_) = bodies.get(entity) {
            ...
            cam.target_radius = scale.m_to_unit_32(
                diameter.ellipsoid.mean_equatorial_radius_km() as f32
                    * 2000.
        ...
        if let Some((entity, _, _, _)) = bodies
            .iter()
            .find(|(_, _, _, maybe_star)| maybe_star.is_some())
        {
